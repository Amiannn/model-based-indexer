...
from dataprocess.utils.trie import Trie
...
class Seq2SeqModule(LightningModule):
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor, decoder_input_ids: torch.Tensor):
        return self.net(x, x_mask, decoder_input_ids)[0]
    ...
    def step(self, batch: Any):
        ...
        logits = self.forward(x_row_idx, x_row_mask, decoder_input_ids)
        loss   = self.criterion(logits.view(-1, logits.size(-1)), y_idx.view(-1))
    ...
    def predict_step(self, batch: Any, batch_idx: int):
        ...
        if self.trie is not None:
            ...
            outputs = self.net.model.generate(
                x_row_idx,
                min_length=0,
                max_length=30,
                num_beams=10,
