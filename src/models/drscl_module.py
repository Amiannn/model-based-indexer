...
class DrsclModule(LightningModule):
    ...
    def __init__(
        ...
        # load trie
        self.trie = None
        if self.net.trie_path != None:
            with open(self.net.trie_path, 'rb') as f:
                trie_dict = pickle.load(f)
            self.trie = Trie.load_from_dict(trie_dict)
    ...
    def _accuracy(self, preds, labels):
        return (torch.sum(torch.floor(torch.sum((preds==labels).int(), axis=1) / labels.size(1))).item()) / labels.size(0)
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor, decoder_input_ids: torch.Tensor):
        return self.net(x, x_mask, decoder_input_ids)
    ...
    def step(self, batch: Any):
        ...
        # teacher forcing
        decoder_input_ids = self.net.model._shift_right(y_idx)

        logits, x_row_vectors = self.forward(x_row_idx, x_row_mask, decoder_input_ids)
        _, x_col_vectors      = self.forward(x_col_idx, x_col_mask, decoder_input_ids)
        ...
        loss_supcon = self._supConLoss(softmax_scores, neg_mask)
        loss_ce     = self.criterion(logits.view(-1, logits.size(-1)), y_idx.view(-1))
        
        alpha = 0.6
        loss  = loss_ce * alpha + (1 - alpha) * loss_supcon
        preds = torch.argmax(logits, dim=2)

        return loss, preds, y_idx
    ...
    def predict_step(self, batch: Any, batch_idx: int):
        ...
        if self.trie is not None:
            ...
            outs    = self.net.model.generate(x, max_length=30)
            outputs = [self.tokenizer.decode(out, skip_special_tokens=True) for out in outs]
        return outputs
